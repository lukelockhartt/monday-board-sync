...
The sync logic:
- Pulls all items from the **Main Board** (set via `SOURCE_BOARD_ID` environment variable).
- Reads a specific column that stores your own internal/client ID.
- Checks the **Duplicate Board** (set via `DEST_BOARD_ID` environment variable) for matching items via `source_item_id`.
...
3. Click **New repository secret** and add these secrets:
   ...
   | `SOURCE_BOARD_ID` | Your source board ID (from the board's URL) |
   | `DEST_BOARD_ID` | Your destination board ID (from the board's URL) |
   | `CLIENT_ID_COLUMN` | The column ID used as a unique client identifier |
